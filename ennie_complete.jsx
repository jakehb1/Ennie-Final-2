  ...
  <>
    ...
    <style>{`
      ...
      *:focus-visible { outline: 2px solid ${T.accent}; outline-offset: 2px; border-radius: 4px; }
      .skip-to-content { position: absolute; left: -9999px; top: 0; z-index: 999; padding: 8px 16px; background: ${T.accent}; color: #fff; font-size: 13px; font-weight: 600; border-radius: 0 0 8px 0; }
      .skip-to-content:focus { left: 0; }
      .ennie-dark img, .ennie-dark video { filter: invert(1) hue-rotate(180deg); }
    `}</style>
  </>
);
...
const TabBar = ({ tabs, active, onTab }) => (
  <div style={{ borderTop: `1px solid ${T.border}`, background: "rgba(255,255,255,0.96)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", position: "sticky", bottom: 0, zIndex: 20, padding: "10px 16px 14px" }}>
    <div role="tablist" aria-label="Navigation" style={{ display: "flex", gap: 6, overflowX: "auto" }}>
      {tabs.map(t => (
        <button key={t.id} role="tab" aria-selected={active === t.id} aria-label={t.label} onClick={() => onTab(t.id)} style={{
...
const BackBtn = ({ onClick }) => (
  <button onClick={onClick} aria-label="Go back" style={{ background: "none", border: "none", color: T.textMuted, fontSize: 20, cursor: "pointer", lineHeight: 1 }}>←</button>
);

/* ─── Empty State ─── */
const EmptyState = ({ icon, title, desc, action, onAction }) => (
  <div style={{ textAlign: "center", padding: "24px 16px", borderRadius: 16, border: `1px dashed ${T.border}`, marginBottom: 14 }}>
    ...
    <p style={{ fontSize: 12, color: T.textMuted, lineHeight: 1.5, marginBottom: action ? 10 : 0 }}>{desc}</p>
    {action && <Btn onClick={onAction} small variant="secondary">{action}</Btn>}
  </div>
);
...
const LiveActivityTicker = () => {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      setFade(false);
      tid = setTimeout(() => { setIdx(i => (i + 1) % ACTIVITY_MESSAGES.length); setFade(true); }, 300);
    }, 4000);
    return () => { clearInterval(iv); clearTimeout(tid); };
  }, []);
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 10, background: T.greenDim, border: `1px solid ${T.green}20`, marginBottom: 14, minHeight: 36 }}>
      <div style={{ width: 8, height: 8, borderRadius: "50%", background: T.green, flexShrink: 0, animation: "pulse 2s ease infinite" }} />
      <span style={{ fontSize: 11, color: T.green, fontWeight: 500, opacity: fade ? 1 : 0, transition: "opacity .3s ease" }}>{ACTIVITY_MESSAGES[idx]}</span>
    </div>
  );
};
...
const RESULTS_DATA = {
  ...
  verified: {
    title: "Paid Sessions (Verified Healers)",
...
const LandingScreen = ({ onGetStarted, onJoinHealer, onLogin }) => {
  ...
  <div style={{ flex: 1, overflowY: "auto", background: "#FFFFFF", display: "flex", flexDirection: "column" }}>
    ...
    <div style={{ padding: "2px 24px 14px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 3, padding: "0 4px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 11, color: T.textMuted }}>Matched with a healer for your condition</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ fontSize: 10, color: T.accent }}>✓</span>
    ...
    <div style={{ padding: "4px 24px 28px" }}>
      ...
      <p style={{ fontSize: 10, color: T.textDim, textAlign: "center", marginBottom: 8 }}>No credit card · Free to start · ~12 minutes</p>
...
const IntakeScreen = ({ onJoinQueue, onPaidSession, onIneligible, pins, setPins, onBack }) => {
  ...
  const reply = (t) => {
    ...
    if (t === "No, not right now") {
      setTimeout(async () => {
        setTyping(true); await wait(1000); setTyping(false);
        setMsgs(m => [...m, { text: "No worries at all. Free sessions need active symptoms so we can measure changes in real time. You're welcome to come back whenever they flare up — or we can show you paid session options.", isAI: true }]);
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          {showQ && (
            <div style={{ padding: "4px 16px 8px" }}>
              <Btn onClick={onJoinQueue} full>Start free session</Btn>
              <div style={{ height: 6 }} />
              <Btn onClick={onPaidSession} variant="ghost" full small>Or book a paid session →</Btn>
            </div>
          )}
  ...
  return (
    <>
      ...
      {showQ && <div style={{ padding: "8px 16px" }}><Btn onClick={onJoinQueue} full>Start free session</Btn><div style={{ height: 6 }} /><Btn onClick={onPaidSession} variant="ghost" full small>Or book a paid session →</Btn></div>}
...
const GroupIntakeScreen = ({ pins, setPins, onDone, onBack }) => {
  const [side, setSide] = useState("front");
  const [selPin, setSelPin] = useState(null);
  // Demo: session starts in 12 minutes
  const [countdown, setCountdown] = useState(720);
  useEffect(() => {
    const i = setInterval(() => setCountdown(t => Math.max(0, t - 1)), 1000);
    return () => clearInterval(i);
  }, []);
  const cMins = Math.floor(countdown / 60), cSecs = countdown % 60;
    ...
    <>
      ...
      <ScreenWrap>
        <div style={{ animation: "slideUp .4s ease" }}>
          ...
          <Card style={{ marginBottom: 14, background: T.purpleDim, border: `1px solid ${T.purple}25`, textAlign: "center", padding: "14px 16px" }}>
            <div style={{ fontSize: 10, fontWeight: 600, color: T.purple, letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>Session starts in</div>
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 32, fontWeight: 800, color: T.purple, letterSpacing: -1 }}>{cMins}:{String(cSecs).padStart(2, "0")}</div>
            <div style={{ fontSize: 11, color: T.textMuted, marginTop: 4 }}>Monday · 7:00 PM · Chronic pain focus</div>
          </Card>

          <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>Mark your symptoms</h2>
          <p style={{ fontSize: 12, color: T.textMuted, marginBottom: 12, lineHeight: 1.5 }}>Place pins where you feel symptoms and rate severity. 1,000 healers will focus on the group during the session.</p>
          ...
          {pins.length === 0 && <div style={{ padding: "16px 14px", borderRadius: 14, border: `1px dashed ${T.border}`, textAlign: "center", marginBottom: 14 }}><p style={{ fontSize: 12, color: T.textDim }}>Tap the body map to mark where you feel symptoms</p></div>}
          <Btn onClick={onDone} full disabled={pins.length === 0} style={pins.length > 0 ? { background: `linear-gradient(135deg, ${T.purple}, ${T.blue})` } : {}}>{pins.length > 0 ? "Join the waiting room →" : "Place at least one pin"}</Btn>
        </div>
      </ScreenWrap>
    </>
  );
...
const GroupWaitingRoom = ({ onStart, onBack }) => {
  const [countdown, setCountdown] = useState(180); // 3 min till session
  const [arrivals, setArrivals] = useState(127);
  const [comments, setComments] = useState([
    { id: 1, name: "Sarah, UK", text: "First time trying group healing!", time: "2 min ago" },
    { id: 2, name: "Marco, Italy", text: "Been looking forward to this all week", time: "1 min ago" },
  ]);
  ...
  useEffect(() => {
    const i = setInterval(() => setCountdown(t => {
      if (t <= 1) { clearInterval(i); onStart(); return 0; }
      return t - 1;
    }), 1000);
    return () => clearInterval(i);
  }, [onStart]);
  ...
  // People arriving
  useEffect(() => {
    const i = setInterval(() => setArrivals(a => Math.min(a + Math.floor(Math.random() * 8 + 2), 580)), 2000);
    return () => clearInterval(i);
  }, []);
  ...
  useEffect(() => {
    const samples = [
      { name: "Yuki, Japan", text: "Hoping for help with my back pain" },
      { name: "Lisa, Canada", text: "This is my 4th session — they keep getting better" },
      { name: "James, US", text: "Let's do this!" },
      { name: "Ana, Brazil", text: "Migraine day. Really hoping this helps" },
      { name: "David, AU", text: "My arthritis has been awful today" },
      { name: "Chen, Singapore", text: "Sending positive energy to everyone here" },
      { name: "Olga, Germany", text: "Can't believe there's 400+ people here" },
      { name: "Raj, India", text: "First time — nervous but hopeful" },
    ];
    let idx = 0;
    const i = setInterval(() => {
      setComments(c => [...c, { ...samples[idx % samples.length], id: Date.now(), time: "just now" }].slice(-8));
      idx++;
  ...
  // Hearts
  useEffect(() => {
    const i = setInterval(() => setHearts(h => [...h.slice(-6), { id: Date.now(), x: 10 + Math.random() * 80 }]), 1500 + Math.random() * 2000);
    return () => clearInterval(i);
  }, []);
  ...
  return (
    <>
      <Header left={<BackBtn onClick={onBack} />} center={<span style={{ fontSize: 13, color: T.purple, fontWeight: 600 }}>Waiting Room</span>} />

      {/* Countdown hero */}
      <div style={{ textAlign: "center", padding: "20px 24px", background: `linear-gradient(180deg, ${T.purpleDim}, ${T.bg})` }}>
        <div style={{ fontSize: 10, fontWeight: 600, color: T.purple, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 6 }}>Session starts in</div>
        <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 52, fontWeight: 800, color: T.purple, letterSpacing: -2, lineHeight: 1 }}>{mins}:{String(secs).padStart(2, "0")}</div>
        <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 12 }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 20, fontWeight: 800, color: T.accent }}>{arrivals}</div>
            <div style={{ fontSize: 9, color: T.textMuted }}>participants</div>
          </div>
          <div style={{ width: 1, background: T.border }} />
          <div style={{ textAlign: "center" }}>
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 20, fontWeight: 800, color: T.green }}>1,000</div>
            <div style={{ fontSize: 9, color: T.textMuted }}>healers ready</div>
          </div>
        </div>
      </div>

      {/* Arrivals animation bar */}
      <div style={{ padding: "0 24px 12px" }}>
        <div style={{ height: 6, background: T.border, borderRadius: 3, overflow: "hidden" }}>
          <div style={{ height: "100%", width: `${(arrivals / 600) * 100}%`, background: `linear-gradient(90deg, ${T.purple}, ${T.accent})`, borderRadius: 3, transition: "width .5s" }} />
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 3 }}>
          <span style={{ fontSize: 9, color: T.textDim }}>Arriving</span>
          <span style={{ fontSize: 9, color: T.textDim }}>~600 expected</span>
        </div>
      </div>
      ...
      <div style={{ padding: "0 24px 12px" }}>
        <Card style={{ padding: "10px 14px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ fontSize: 11, fontWeight: 600 }}>Joining from around the world</span>
            <Badge color={T.purple}>{arrivals} connected</Badge>
          </div>
          <div style={{ position: "relative", height: 65, background: T.surface, borderRadius: 10, overflow: "hidden", border: `1px solid ${T.border}` }}>
            {[
              { x: 25, y: 30, count: Math.round(arrivals * 0.27) },
              { x: 48, y: 25, count: Math.round(arrivals * 0.23) },
              { x: 55, y: 42, count: Math.round(arrivals * 0.07) },
              { x: 65, y: 28, count: Math.round(arrivals * 0.32) },
              { x: 82, y: 55, count: Math.round(arrivals * 0.11) },
            ].map((r, i) => (
              <div key={i} style={{ position: "absolute", left: `${r.x}%`, top: `${r.y}%`, transform: "translate(-50%,-50%)" }}>
                <div style={{ width: Math.max(10, r.count / 8), height: Math.max(10, r.count / 8), borderRadius: "50%", background: T.purple + "25", border: `1.5px solid ${T.purple}40`, display: "flex", alignItems: "center", justifyContent: "center", transition: "all .5s" }}>
                  <span style={{ fontSize: 6, fontWeight: 800, color: T.purple }}>{r.count}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Live community chat */}
      <div style={{ flex: 1, padding: "0 24px", overflowY: "auto", position: "relative" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          <span style={{ fontSize: 12, fontWeight: 600 }}>Community</span>
          <span style={{ fontSize: 10, color: T.textDim }}>{comments.length} messages</span>
        </div>

        {/* Floating hearts */}
        {hearts.map(h => (
          <div key={h.id} style={{ position: "absolute", left: `${h.x}%`, bottom: 10, fontSize: 16, animation: "floatUp 3s ease-out forwards", opacity: 0, pointerEvents: "none", zIndex: 2 }}>💜</div>
        ))}

        {comments.map((c, i) => (
          <div key={c.id} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "6px 0", borderBottom: i < comments.length - 1 ? `1px solid ${T.border}` : "none", animation: i === comments.length - 1 ? "slideUp .3s ease" : "none" }}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: T.accentDim, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, flexShrink: 0 }}>🌍</div>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: T.text }}>{c.name}</span>
                <span style={{ fontSize: 9, color: T.textDim }}>{c.time}</span>
              </div>
              <p style={{ fontSize: 12, color: T.textMuted, marginTop: 1 }}>{c.text}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Session info footer */}
      <div style={{ padding: "10px 24px 16px", borderTop: `1px solid ${T.border}`, background: T.surface }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "center" }}>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: T.purple, animation: "pulse 2s infinite" }} />
          <span style={{ fontSize: 11, color: T.purple, fontWeight: 500 }}>1,000 healers are standing by · 30 min session · Chronic pain focus</span>
        </div>
      </div>
...
const GroupSessionScreen = ({ pins, setPins, onEnd }) => {
  ...
  const [improvementPct, setImprovementPct] = useState(8);
  const [hearts, setHearts] = useState([]);
  const [liveComments, setLiveComments] = useState([]);
  const [viewMode, setViewMode] = useState("body"); // body | community
  const ref = useRef(null);

  // Sequential AI messages
  const scriptRef = useRef([
    { text: `Welcome to group healing. ${participantCount} participants connected. 1,000 healers are beginning now.`, pause: 5000 },
    { text: "Focus on the areas you've marked. Update your body map as you notice any changes.", pause: 15000 },
    { text: "How are your symptoms feeling? Use the quick buttons below to report.", pause: 25000 },
    { text: "Multiple participants are reporting changes. Keep tracking — even subtle shifts matter.", pause: 30000 },
    { text: "Halfway. The healers are intensifying focus. Update your severity if anything has changed.", pause: 30000 },
    { text: "Final stretch. Compare how you feel now versus the start.", pause: 30000 },
    { text: "Last minute. Thank you for contributing your data.", pause: 0 },
  ]);
  const scriptIdx = useRef(0);

  const runNextMsg = useCallback(async () => {
    if (scriptIdx.current >= scriptRef.current.length) return;
    const s = scriptRef.current[scriptIdx.current];
    setTyping(true); await wait(1500); setTyping(false);
    setMsgs(prev => [...prev, { text: s.text, isAI: true }]);
    scriptIdx.current++;
    if (s.pause > 0 && scriptIdx.current < scriptRef.current.length) {
      setTimeout(runNextMsg, s.pause);
    }
  }, []);

  useEffect(() => { runNextMsg(); }, [runNextMsg]);
  useEffect(() => { ref.current?.scrollIntoView({ behavior: "smooth" }); }, [msgs, typing]);
  useEffect(() => { const i = setInterval(() => setTimer(t => { if (t <= 1) { clearInterval(i); onEndRef.current(); return 0; } return t - 1; }), 1000); return () => clearInterval(i); }, []);

  // Simulate group improvement
  useEffect(() => {
    const i = setInterval(() => setImprovementPct(p => Math.min(p + Math.random() * 3, 65)), 5000);
    return () => clearInterval(i);
  }, []);

  // Simulate hearts
  useEffect(() => {
    const i = setInterval(() => setHearts(h => [...h.slice(-6), { id: Date.now(), x: 15 + Math.random() * 70 }]), 2500 + Math.random() * 3000);
    return () => clearInterval(i);
  }, []);
  ...
  useEffect(() => {
    const samples = [
      { name: "Sarah, UK", text: "My headache is fading..." },
      { name: "Marco, Italy", text: "Feeling warmth in my back" },
      { name: "Yuki, Japan", text: "Something is shifting" },
      { name: "Lisa, Canada", text: "This is incredible" },
      { name: "David, AU", text: "Down from 7 to 4" },
      { name: "Ana, Brazil", text: "My anxiety is lifting" },
    ];
    let idx = 0;
    const i = setInterval(() => {
      setLiveComments(c => [...c.slice(-4), { ...samples[idx % samples.length], id: Date.now() }]);
      idx++;
  ...
  return (
    <>
      ...
      {/* Group stats bar */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 16px", background: T.purpleDim, borderBottom: `1px solid ${T.purple}20` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: T.purple, animation: "pulse 2s infinite" }} />
          <span style={{ fontSize: 10, color: T.purple, fontWeight: 500 }}>1,000 healers · {participantCount} participants</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: T.green }}>{Math.round(improvementPct)}%</span>
          <span style={{ fontSize: 9, color: T.textMuted }}>improving</span>
        </div>
      </div>

      {/* View toggle — body map vs community */}
      <div style={{ display: "flex", gap: 3, padding: "6px 16px", background: T.surface, borderBottom: `1px solid ${T.border}` }}>
        {[{ id: "body", label: "Your symptoms" }, { id: "community", label: "Community" }].map(v => (
          <button key={v.id} onClick={() => setViewMode(v.id)} style={{ flex: 1, padding: "6px 4px", borderRadius: 8, border: "none", fontSize: 11, fontWeight: 600, cursor: "pointer", background: viewMode === v.id ? T.card : "transparent", color: viewMode === v.id ? T.purple : T.textMuted, transition: "all .15s" }}>{v.label}</button>
        ))}
      </div>

      {/* Body map view */}
      {viewMode === "body" && (
        <div style={{ flex: "0 0 auto", maxHeight: "35vh", overflowY: "auto", background: T.surface, borderBottom: `1px solid ${T.border}`, padding: "4px 12px 8px" }}>
      ...
      {/* Community view */}
      {viewMode === "community" && (
        <div style={{ flex: "0 0 auto", maxHeight: "35vh", overflowY: "auto", background: T.surface, borderBottom: `1px solid ${T.border}`, padding: "8px 14px" }}>
          {/* World map */}
          <div style={{ position: "relative", height: 60, background: T.bg, borderRadius: 10, overflow: "hidden", border: `1px solid ${T.border}`, marginBottom: 8 }}>
            {[
              { x: 25, y: 30, count: 156 },
              { x: 48, y: 25, count: 134 },
              { x: 55, y: 40, count: 42 },
              ...
              <div key={i} style={{ position: "absolute", left: `${r.x}%`, top: `${r.y}%`, transform: "translate(-50%,-50%)", width: Math.max(10, r.count / 10), height: Math.max(10, r.count / 10), borderRadius: "50%", background: T.purple + "30", border: `1px solid ${T.purple}50`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ fontSize: 6, fontWeight: 800, color: T.purple }}>{r.count}</span>
              </div>
            ))}
          </div>

          {/* Live comments + hearts */}
          <div style={{ position: "relative", minHeight: 80 }}>
            {hearts.map(h => (
              <div key={h.id} style={{ position: "absolute", left: `${h.x}%`, bottom: 0, fontSize: 14, animation: "floatUp 3s ease-out forwards", opacity: 0, pointerEvents: "none" }}>💜</div>
            ))}
            {liveComments.map((c, i) => (
              <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 0", animation: "slideUp .3s ease" }}>
                <span style={{ fontSize: 10 }}>🌍</span>
                <span style={{ fontSize: 9, color: T.textDim }}>{c.name}</span>
                <span style={{ fontSize: 11, color: T.text }}>"{c.text}"</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <style>{`@keyframes floatUp { 0% { transform:translateY(0); opacity:1 } 100% { transform:translateY(-60px); opacity:0 } }`}</style>
    </>
  );
};
...
const ConnectingScreen = ({ onConnected, isGroup, onCancel }) => {
  ...
  useEffect(() => {
    if (cancelled) return;
    const timers = [setTimeout(() => setStep(1), 1500), setTimeout(() => setStep(2), 3500), setTimeout(() => setStep(3), 5500), setTimeout(() => onConnectedRef.current(), 7000)];
...
const LiveSessionScreen = ({ pins, setPins, baselinePins, onEnd }) => {
  ...
  useEffect(() => {
    if (!active) return;
    const i = setInterval(() => setTimer(t => { if (t <= 1) { clearInterval(i); onEndRef.current(); return 0; } return t - 1; }), 1000);
    return () => clearInterval(i);
  }, [active]);
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
const SessionEndScreen = ({ baselinePins: _bp, finalPins: _fp, isGroup, onHome, onThankYou, onNoResult }) => {
  const baselinePins = _bp && _bp.length > 0 ? _bp : [{ id: 1, x: 52, y: 55, side: "front", severity: 5 }];
  const finalPins = _fp && _fp.length > 0 ? _fp : baselinePins.map(p => ({ ...p }));
  ...
  <>
    ...
    <ScreenWrap>
      <div style={{ textAlign: "center", marginBottom: 16, animation: "slideUp .5s ease" }}>
        ...
        <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 4 }}>{improved ? (isGroup ? "Group session — your symptoms improved" : "Your symptoms improved") : (isGroup ? "Group session complete" : "Session complete")}</h2>
        <p style={{ fontSize: 13, color: T.textMuted }}>{improved ? `A total drop of ${totalDrop} points across ${baselinePins.length} symptom${baselinePins.length > 1 ? "s" : ""}.${isGroup ? " You were one of 500+ participants." : ""}` : (isGroup ? "Let's look at what happened in the group session." : "Let's look at what happened.")}</p>
      </div>
      ...
      {improved && (() => {
        const avgDrop = 3.2;
        const pctDrop = Math.round((totalDrop / Math.max(baselinePins.reduce((s, p) => s + p.severity, 0), 1)) * 100);
        const isBest = totalDrop >= 5;
        const aboveAvg = totalDrop > avgDrop ? Math.round(((totalDrop - avgDrop) / avgDrop) * 100) : 0;
        return (
          <Card style={{ marginBottom: 12, marginTop: 4, textAlign: "center", background: isBest ? `linear-gradient(135deg, ${T.greenDim}, ${T.purpleDim})` : T.accentDim, border: `1px solid ${isBest ? T.green : T.accent}20`, padding: "12px 14px" }}>
            <span style={{ fontSize: 16, marginRight: 6 }}>{isBest ? "🏆" : "📈"}</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: isBest ? T.green : T.accent }}>{isBest ? "Your best session yet!" : aboveAvg > 0 ? `${aboveAvg}% better than your average` : "Matching your average"}</span>
            {!isBest && <div style={{ fontSize: 11, color: T.textMuted, marginTop: 2 }}>Avg drop: {avgDrop} pts · This session: {totalDrop} pts</div>}
          </Card>
        );
      })()}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: T.textMuted }}>Every person your ad brings to Ennie earns you credit toward future sessions — Paid Sessions, Group Healing, anything.</div>
                  </div>
                </div>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
...
const FollowUpScreen = ({ onPositive, onNeutral, pins, setPins }) => {
  ...
  const onPositiveRef = useRef(onPositive);
  onPositiveRef.current = onPositive;
  useEffect(() => { if (submitted) { const t = setTimeout(() => onPositiveRef.current(), 2000); return () => clearTimeout(t); } }, [submitted]);
  if (submitted) return (
    <>
      <Header left={<Logo />} center={<span style={{ fontSize: 13, color: T.textMuted, fontWeight: 500 }}>Follow-up</span>} />
      <ScreenWrap>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", animation: "slideUp .4s ease", textAlign: "center" }}>
          <div style={{ width: 64, height: 64, borderRadius: "50%", background: T.greenDim, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, marginBottom: 16, border: `2px solid ${T.green}40` }}>✓</div>
          <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 6 }}>Check-in recorded</h2>
          <p style={{ fontSize: 13, color: T.textMuted, maxWidth: 260 }}>Thanks for logging your follow-up. This helps us measure long-term outcomes.</p>
        </div>
      </ScreenWrap>
    </>
  );
    ...
    <>
      ...
      <ScreenWrap>
        <div style={{ animation: "slideUp .4s ease" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {[{ label: "100% better", icon: "✅", v: "accent", fn: "pos" }, { label: "It didn't help", icon: "✕", v: "secondary", fn: "neut" }].map(r => <Btn key={r.label} variant={r.v} small onClick={r.fn === "pos" ? () => setSubmitted(true) : onNeutral} style={{ flex: 1 }}>{r.icon} {r.label}</Btn>)}
          </div>
          <Btn onClick={() => setSubmitted(true)} full>Submit check-in →</Btn>
        </div>
      </ScreenWrap>
    </>
  );
  ...
  const supportScript = useMemo(() => {
    ...
    return [
      ...
      ...(paid ? [
        ...
        { ai: `For ${condition.label.toLowerCase()}, about ${successRate}% of free sessions show measurable improvement. That's based on real patient feedback, validated by UC Irvine. It's encouraging, but it means not everyone will respond — especially in a single session.`, delay: 1200 },
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "0 24px", animation: "fadeIn .6s ease" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 20, textAlign: "left" }}>
            {[
              { icon: "🔒", title: "Anonymous & AI-mediated", desc: "You never speak to the patient. Our AI handles all communication. Both sides are completely anonymous." },
              { icon: "📊", title: "Every session is measured", desc: "Patient symptoms are tracked before, during, and after. Your results determine your qualification and pay." },
              { icon: "🌍", title: "Remote & flexible", desc: "Heal from wherever you are. Sessions are part of ongoing UCI research — your work has real impact." },
            ].map((item, i) => (
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
const pastGroupSessions = [
  { id: 1, date: "Mar 27", day: "Thursday", time: "8:00 PM", participants: 512, improved: 64, avgDrop: 3.8, earned: 100, focus: "Chronic pain" },
  { id: 2, date: "Mar 24", day: "Monday", time: "7:00 PM", participants: 487, improved: 61, avgDrop: 4.1, earned: 100, focus: "Chronic pain" },
  { id: 3, date: "Mar 20", day: "Thursday", time: "8:00 PM", participants: 534, improved: 66, avgDrop: 4.3, earned: 100, focus: "Chronic pain" },
  { id: 4, date: "Mar 17", day: "Monday", time: "7:00 PM", participants: 498, improved: 59, avgDrop: 3.5, earned: 100, focus: "Chronic pain" },
];

const GroupSessionHistory = () => {
  const [expanded, setExpanded] = useState(false);
  const [detail, setDetail] = useState(null);
  const totalEarned = pastGroupSessions.reduce((s, g) => s + g.earned, 0);
  const avgImproved = Math.round(pastGroupSessions.reduce((s, g) => s + g.improved, 0) / pastGroupSessions.length);

  return (
    <Card style={{ marginBottom: 12 }}>
      <div onClick={() => setExpanded(!expanded)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}>
        <div>
          <div style={{ fontSize: 12, fontWeight: 600 }}>Past group sessions</div>
          <div style={{ fontSize: 10, color: T.textMuted, marginTop: 2 }}>{pastGroupSessions.length} sessions · ${totalEarned} earned · {avgImproved}% avg improvement</div>
        </div>
      ...
      {expanded && (
        <div style={{ marginTop: 10, animation: "slideUp .2s ease" }}>
          {pastGroupSessions.map((g, i) => (
            <div key={g.id}>
              <div onClick={() => setDetail(detail === g.id ? null : g.id)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderTop: `1px solid ${T.border}`, cursor: "pointer" }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ fontSize: 12, fontWeight: 600 }}>{g.day}</span>
                    <span style={{ fontSize: 11, color: T.purple }}>{g.time}</span>
                    <span style={{ fontSize: 10, color: T.textDim }}>· {g.date}</span>
                  </div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: T.green }}>{g.improved}%</span>
                  <span style={{ fontSize: 10, color: T.green }}>$100</span>
                  <span style={{ fontSize: 10, color: T.textMuted }}>{detail === g.id ? "▴" : "▾"}</span>
                </div>
              </div>

              {detail === g.id && (
                <div style={{ padding: "8px 0 8px 4px", animation: "slideUp .2s ease" }}>
                  <div style={{ display: "flex", gap: 0, padding: "8px 0", borderRadius: 10, background: T.surface, marginBottom: 6 }}>
                    {[
                      { label: "Participants", value: g.participants, color: T.purple },
                      { label: "Improved", value: `${g.improved}%`, color: T.green },
                      { label: "Avg drop", value: g.avgDrop.toFixed(1), color: T.accent },
                    ].map((s, j) => (
                      <div key={j} style={{ flex: 1, textAlign: "center", borderRight: j < 2 ? `1px solid ${T.border}` : "none" }}>
                        <div style={{ fontSize: 14, fontWeight: 800, color: s.color }}>{s.value}</div>
                        <div style={{ fontSize: 8, color: T.textMuted }}>{s.label}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{ display: "flex", gap: 4, alignItems: "flex-end", height: 24, marginBottom: 4 }}>
                    {[8, 15, 25, 35, 42, 50, 55, g.improved].map((v, j) => (
                      <div key={j} style={{ flex: 1, height: `${(v / 70) * 100}%`, background: T.green + "60", borderRadius: 2, minHeight: 2 }} />
                    ))}
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 7, color: T.textDim }}>Start</span>
                    <span style={{ fontSize: 7, color: T.textDim }}>End</span>
                  </div>
                </div>
              )}
            </div>
...
const HealerHomeScreen = ({ onGoOnline, onSpecializations, onGroupInvite, onReferrals }) => {
  ...
  return (
    <ScreenWrap>
      <div style={{ animation: "slideUp .4s ease" }}>
        ...
        {/* Group Healing Invitation — shown to top healers */}
        {OVERALL_STATS.pct >= 70 && (
          <>
            <Divider />
            <Label>GROUP HEALING</Label>
            <Card style={{ marginBottom: 12, border: `2px solid ${T.purple}35`, background: `${T.purple}06`, cursor: "pointer" }} onClick={onGroupInvite}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                <div style={{ width: 48, height: 48, borderRadius: 14, background: `linear-gradient(135deg, ${T.purpleDim}, ${T.accentDim})`, border: `2px solid ${T.purple}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>👥</div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: T.purple }}>You've been invited</span>
                    <Badge color={T.purple}>New</Badge>
                  </div>
                  <div style={{ fontSize: 12, color: T.textMuted }}>Join our top 1,000 healers in group sessions</div>
                </div>
                <span style={{ color: T.purple, fontSize: 16 }}>→</span>
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                {[
                  { label: "$100/session", color: T.green },
                  { label: "30 min each", color: T.purple },
                  { label: "400–600 participants", color: T.blue },
                ].map((s, i) => (
                  <div key={i} style={{ padding: "4px 8px", borderRadius: 8, background: s.color + "12", border: `1px solid ${s.color}20` }}>
            ...
            {/* Committed sessions */}
            <Card style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Your group sessions</div>
              {[
                { day: "Monday", time: "7:00 PM", status: "join", pay: "$100" },
                { day: "Thursday", time: "8:00 PM", status: "confirmed", pay: "$100" },
              ].map((s, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: i < 1 ? `1px solid ${T.border}` : "none" }}>
                  <div>
                    <span style={{ fontSize: 13, fontWeight: 600 }}>{s.day}</span>
                    <span style={{ fontSize: 12, color: T.purple, marginLeft: 8, fontWeight: 600 }}>{s.time}</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ fontSize: 11, color: T.green, fontWeight: 600 }}>{s.pay}</span>
                    {s.status === "join" ? (
                      <button onClick={(e) => { e.stopPropagation(); onGroupInvite("joinSession"); }} style={{ padding: "4px 12px", borderRadius: 8, border: "none", background: `linear-gradient(135deg, ${T.purple}, ${T.blue})`, color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer", animation: "breathe 2s ease-in-out infinite" }}>Join now</button>
                    ) : (
                      <Badge color={T.green}>Confirmed</Badge>
                    )}
                  </div>
            ...
            {/* Past group sessions — expandable */}
            <GroupSessionHistory />
          </>
        )}
        ...
        {/* Referral card */}
        <Card onClick={onReferrals} style={{ marginBottom: 12, cursor: "pointer", border: `1px solid ${T.green}25`, background: `${T.green}04` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12, fontWeight: 600 }}>Refer & earn</div>
              <div style={{ fontSize: 10, color: T.textMuted }}>$25 per patient · $500 per healer</div>
            </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 16px" }}>
        <div style={{ width: "100%", maxWidth: 360, animation: claimed ? "fadeIn .2s" : "slideUp .4s ease" }}>
          ...
          <Card style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontSize: 12, color: T.textMuted }}>Session type</span><span style={{ fontSize: 13 }}>Remote · 5 min clock (resets on improvement)</span></div>
          </Card>
...
const HealerEarningsScreen = () => {
  ...
  const groupEarned = 1200;
  const allTimeEarned = totalEarned + groupEarned;
  const thisMonth = blocks.filter(b => b.date.includes("Mar")).reduce((s, b) => s + b.payout, 0) + 500; // includes group sessions this month
    ...
    <ScreenWrap>
      <div style={{ animation: "slideUp .4s ease" }}>
        ...
        <div style={{ textAlign: "center", padding: "12px 0 20px" }}>
          ...
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 40, fontWeight: 800, letterSpacing: -2, color: T.text }}>${allTimeEarned.toLocaleString()}</div>
        ...
        <Card style={{ marginTop: 8, marginBottom: 16, background: `linear-gradient(135deg, ${T.accentDim}, ${T.purpleDim})`, border: `1px solid ${T.accent}20`, textAlign: "center", padding: 20 }}>
          <div style={{ fontSize: 28, marginBottom: 6 }}>💜</div>
          <p style={{ fontSize: 14, fontWeight: 600, lineHeight: 1.5 }}>You've earned ${(totalEarned + 1200).toLocaleString()} across 1:1 sessions and group healing — while helping real people with real pain.</p>
          <p style={{ fontSize: 12, color: T.textMuted, marginTop: 4 }}>That's what healing as a profession looks like.</p>
        </Card>

        <Divider />

        {/* Group Healing Earnings */}
        <Label>GROUP HEALING EARNINGS</Label>
        <Card style={{ marginBottom: 12, border: `1px solid ${T.purple}20`, background: `${T.purple}04` }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: T.purpleDim, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>👥</div>
              <div>
                <div style={{ fontSize: 14, fontWeight: 700 }}>Group sessions</div>
                <div style={{ fontSize: 11, color: T.textMuted }}>$100 per session · Paid within 48 hours</div>
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 20, fontWeight: 800, color: T.purple }}>$1,200</div>
              <div style={{ fontSize: 9, color: T.textMuted }}>total earned</div>
            </div>
          </div>

          {/* Recent group sessions */}
          {[
            { date: "Mon, Mar 31", time: "7:00 PM", participants: 512, improved: "63%", avgDrop: "4.1", pay: "$100", status: "paid" },
            { date: "Thu, Mar 27", time: "8:00 PM", participants: 487, improved: "59%", avgDrop: "3.8", pay: "$100", status: "paid" },
            { date: "Mon, Mar 24", time: "7:00 PM", participants: 534, improved: "65%", avgDrop: "4.3", pay: "$100", status: "paid" },
            { date: "Thu, Mar 20", time: "8:00 PM", participants: 491, improved: "61%", avgDrop: "3.9", pay: "$100", status: "paid" },
            { date: "Mon, Mar 17", time: "7:00 PM", participants: 523, improved: "64%", avgDrop: "4.2", pay: "$100", status: "paid" },
          ].map((s, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderTop: `1px solid ${T.border}` }}>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 12, fontWeight: 600 }}>{s.date}</span>
                  <span style={{ fontSize: 11, color: T.purple }}>{s.time}</span>
                </div>
                <div style={{ fontSize: 10, color: T.textMuted, marginTop: 1 }}>{s.participants} participants · {s.improved} improved · −{s.avgDrop}pts avg</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: T.green }}>{s.pay}</span>
                <div style={{ fontSize: 9, color: T.green }}>✓ {s.status}</div>
              </div>
            </div>
          ))}
        </Card>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          {[
            { value: "12", label: "Sessions", color: T.purple },
            { value: "6,142", label: "People helped", color: T.green },
            { value: "62%", label: "Avg improved", color: T.accent },
          ].map((s, i) => (
            <div key={i} style={{ flex: 1, textAlign: "center", padding: "10px 6px", borderRadius: 12, background: s.color + "08", border: `1px solid ${s.color}15` }}>
...
/* ═══════════════════════════════════════════════════════════════
   HEALER GROUP HEALING — Invite + Commit + Live Session
   ═══════════════════════════════════════════════════════════════ */
const HealerGroupInviteScreen = ({ onCommit, onBack }) => {
  const [selectedSessions, setSelectedSessions] = useState([]);
  const [committed, setCommitted] = useState(false);
  const toggle = (id) => setSelectedSessions(s => s.includes(id) ? s.filter(x => x !== id) : [...s, id]);
  const availableSessions = [
    { id: 1, day: "Monday", time: "7:00 PM", focus: "Chronic pain", spots: "3 healer spots left" },
    { id: 2, day: "Wednesday", time: "12:00 PM", focus: "Stress & anxiety", spots: "5 healer spots left" },
    { id: 3, day: "Thursday", time: "8:00 PM", focus: "General wellness", spots: "Full" },
    { id: 4, day: "Saturday", time: "10:00 AM", focus: "Chronic pain", spots: "8 healer spots left" },
    { id: 5, day: "Sunday", time: "6:00 PM", focus: "Migraine & headaches", spots: "2 healer spots left" },
    { id: 6, day: "Friday", time: "1:00 PM", focus: "General wellness", spots: "12 healer spots left" },
  ];
  return (
    <>
      <Header left={<BackBtn onClick={onBack} />} center={<span style={{ fontSize: 13, color: T.textMuted, fontWeight: 500 }}>Group Healing</span>} />
      <ScreenWrap>
        <div style={{ animation: "slideUp .4s ease" }}>
          {/* Hero */}
          <div style={{ textAlign: "center", padding: "12px 0 16px" }}>
            <div style={{ width: 64, height: 64, borderRadius: 18, background: `linear-gradient(135deg, ${T.purpleDim}, ${T.accentDim})`, border: `2px solid ${T.purple}30`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px", fontSize: 28 }}>👥</div>
            <h2 style={{ fontSize: 22, fontWeight: 800, marginBottom: 4 }}>You've been selected</h2>
            <p style={{ fontSize: 13, color: T.textMuted, lineHeight: 1.5, maxWidth: 300, margin: "0 auto" }}>Your performance qualifies you to join our top 1,000 healers in group healing sessions.</p>
          </div>

          {/* The deal */}
          <Card style={{ marginBottom: 14, background: `linear-gradient(135deg, ${T.greenDim}, ${T.accentDim})`, border: `1px solid ${T.green}25` }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: T.green, marginBottom: 8 }}>What you get</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                <span style={{ fontSize: 14 }}>💰</span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>$100 per session</div>
                  <div style={{ fontSize: 11, color: T.textMuted }}>Paid within 48 hours of each completed session</div>
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                <span style={{ fontSize: 14 }}>⏱️</span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>30 minutes per session</div>
                  <div style={{ fontSize: 11, color: T.textMuted }}>You work alongside 999 other top healers simultaneously</div>
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                <span style={{ fontSize: 14 }}>📊</span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>Live impact data</div>
                  <div style={{ fontSize: 11, color: T.textMuted }}>See real-time improvement across 400–600 participants during the session</div>
                </div>
              </div>
            </div>
          </Card>
          ...
          {/* How it works */}
          <Card style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 8 }}>How group sessions work</div>
            {[
              { num: "1", text: "Commit to one or more weekly sessions below" },
              { num: "2", text: "At session time, join the group healing room" },
              { num: "3", text: "Focus your intention on the group for 30 minutes" },
              { num: "4", text: "See the real-time data — how many people improved, by how much" },
              { num: "5", text: "Get paid $100 per completed session" },
            ].map((s, i) => (
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "5px 0", borderBottom: i < 4 ? `1px solid ${T.border}` : "none" }}>
                <div style={{ width: 20, height: 20, borderRadius: 6, background: T.purple + "18", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: T.purple, flexShrink: 0, marginTop: 1 }}>{s.num}</div>
                <span style={{ fontSize: 12, color: T.textMuted, lineHeight: 1.45 }}>{s.text}</span>
              </div>
            ))}
          </Card>
          ...
          {/* Available sessions */}
          <Label>SELECT SESSIONS TO COMMIT</Label>
          {availableSessions.map((s, i) => {
            const isSel = selectedSessions.includes(s.id);
            const isFull = s.spots === "Full";
            return (
              <Card key={s.id} onClick={() => !isFull && toggle(s.id)} style={{ marginBottom: 8, cursor: isFull ? "default" : "pointer", opacity: isFull ? 0.5 : 1, border: `1px solid ${isSel ? T.purple + "50" : T.border}`, background: isSel ? T.purpleDim : T.card }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 20, height: 20, borderRadius: 6, border: `2px solid ${isSel ? T.purple : T.border}`, background: isSel ? T.purple : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: T.bg }}>{isSel ? "✓" : ""}</div>
                    <span style={{ fontSize: 14, fontWeight: 600 }}>{s.day}</span>
                    <span style={{ fontSize: 13, color: T.purple, fontWeight: 600 }}>{s.time}</span>
                  </div>
                  <Badge color={isFull ? T.textMuted : T.purple} bg={isFull ? T.surface : T.purpleDim}>{s.focus}</Badge>
                </div>
                <div style={{ fontSize: 10, color: T.textDim, marginTop: 4, paddingLeft: 28 }}>{s.spots}</div>
              </Card>
            );
          })}
          ...
          <div style={{ height: 12 }} />
          {committed ? (
            <Card style={{ textAlign: "center", padding: "20px 16px", background: T.greenDim, border: `1px solid ${T.green}25`, animation: "slideUp .3s ease" }}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>✅</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: T.green, marginBottom: 4 }}>You're committed!</div>
              <p style={{ fontSize: 12, color: T.textMuted, marginBottom: 12 }}>{selectedSessions.length} session{selectedSessions.length > 1 ? "s" : ""} confirmed — ${selectedSessions.length * 100}/week. You'll see them on your dashboard.</p>
              <Btn onClick={onCommit} full>Back to dashboard →</Btn>
            </Card>
          ) : (
            <>
              <Btn onClick={() => setCommitted(true)} full disabled={selectedSessions.length === 0} style={selectedSessions.length > 0 ? { background: `linear-gradient(135deg, ${T.purple}, ${T.blue})` } : {}}>
                {selectedSessions.length > 0 ? `Commit to ${selectedSessions.length} session${selectedSessions.length > 1 ? "s" : ""} — $${selectedSessions.length * 100}/week` : "Select sessions to commit"}
              </Btn>
              <p style={{ fontSize: 10, color: T.textDim, textAlign: "center", marginTop: 6 }}>You can change your commitments anytime from your dashboard</p>
            </>
          )}
        </div>
      </ScreenWrap>
...
const HealerGroupSessionScreen = ({ onEnd }) => {
  const [timer, setTimer] = useState(1800);
  const [participantCount] = useState(Math.floor(Math.random() * 200) + 400);
  const [healerCount] = useState(Math.floor(Math.random() * 50) + 950);
  const [improvementPct, setImprovementPct] = useState(12);
  const [avgDrop, setAvgDrop] = useState(0.8);
  const [hearts, setHearts] = useState([]);
  const [comments, setComments] = useState([]);
  const [showComments, setShowComments] = useState(true);

  // Simulate improving stats over time
  useEffect(() => {
    const i = setInterval(() => {
      setImprovementPct(p => Math.min(p + Math.random() * 3, 68));
      setAvgDrop(d => Math.min(d + Math.random() * 0.15, 4.2));
    }, 5000);
    return () => clearInterval(i);
  }, []);
  ...
  useEffect(() => {
    const i = setInterval(() => {
      setHearts(h => [...h.slice(-8), { id: Date.now(), x: 20 + Math.random() * 60 }]);
  ...
  useEffect(() => {
    const sampleComments = [
      ...
      { name: "Marco, Italy", text: "Feeling warmth in my back" },
      { name: "Yuki, Japan", text: "Something is shifting" },
      { name: "Anonymous", text: "The pain is definitely less" },
      { name: "Lisa, Canada", text: "This is incredible" },
      { name: "David, Australia", text: "I can feel it working" },
      { name: "Ana, Brazil", text: "My anxiety is lifting" },
      { name: "James, US", text: "Down from 7 to 4 already" },
    ];
    let idx = 0;
    const i = setInterval(() => {
      setComments(c => [...c.slice(-5), { ...sampleComments[idx % sampleComments.length], id: Date.now() }]);
      idx++;
  ...
  const mins = Math.floor(timer / 60), secs = timer % 60;
  const elapsed = 1800 - timer;
  const phase = elapsed < 300 ? "Opening" : elapsed < 900 ? "Deep focus" : elapsed < 1500 ? "Intensifying" : "Final stretch";

  return (
    <>
      <Header
        left={<div style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ fontSize: 12 }}>👥</span><span style={{ fontSize: 11, color: T.purple, fontWeight: 600 }}>{healerCount}</span></div>}
        center={<span style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 800, color: timer < 60 ? T.warm : T.text }}>{mins}:{String(secs).padStart(2, "0")}</span>}
        right={<Btn variant="danger" small onClick={onEnd} style={{ padding: "5px 12px" }}>End</Btn>}
      />

      {/* Phase + participant bar */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 16px", background: T.purpleDim, borderBottom: `1px solid ${T.purple}20` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: T.purple, animation: "pulse 2s infinite" }} />
          <span style={{ fontSize: 10, color: T.purple, fontWeight: 600 }}>{phase}</span>
        </div>
        <span style={{ fontSize: 10, color: T.purple }}>{participantCount} participants</span>
      </div>
      ...
      <ScreenWrap>
        <div style={{ animation: "slideUp .4s ease" }}>
          {/* Live stats */}
          <Card style={{ marginBottom: 12, border: `1px solid ${T.purple}20` }}>
            <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 10, color: T.purple }}>Live group impact</div>
            <div style={{ display: "flex", gap: 0, marginBottom: 10 }}>
              <div style={{ flex: 1, textAlign: "center", borderRight: `1px solid ${T.border}` }}>
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 24, fontWeight: 800, color: T.green }}>{Math.round(improvementPct)}%</div>
                <div style={{ fontSize: 9, color: T.textMuted }}>reporting improvement</div>
              </div>
              <div style={{ flex: 1, textAlign: "center", borderRight: `1px solid ${T.border}` }}>
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 24, fontWeight: 800, color: T.accent }}>{avgDrop.toFixed(1)}</div>
                <div style={{ fontSize: 9, color: T.textMuted }}>avg severity drop</div>
              </div>
              <div style={{ flex: 1, textAlign: "center" }}>
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 24, fontWeight: 800, color: T.purple }}>{participantCount}</div>
                <div style={{ fontSize: 9, color: T.textMuted }}>participants</div>
              </div>
            </div>

            {/* Progress over time */}
            <div style={{ display: "flex", gap: 4, alignItems: "flex-end", height: 30, marginBottom: 6 }}>
              {Array.from({ length: 10 }).map((_, i) => {
                const val = i <= Math.floor(elapsed / 180) ? (12 + i * 5.5 + Math.random() * 3) : 0;
                return <div key={i} style={{ flex: 1, height: `${(val / 70) * 100}%`, background: val > 0 ? `linear-gradient(180deg, ${T.green}90, ${T.green}30)` : T.border, borderRadius: 2, minHeight: val > 0 ? 3 : 1, transition: "height .5s" }} />;
              })}
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ fontSize: 8, color: T.textDim }}>Start</span>
              <span style={{ fontSize: 8, color: T.textDim }}>Now</span>
              <span style={{ fontSize: 8, color: T.textDim }}>End</span>
            </div>
          </Card>

          {/* World map — simplified dot map */}
          <Card style={{ marginBottom: 12, padding: "12px 14px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
              <span style={{ fontSize: 12, fontWeight: 600 }}>Participants worldwide</span>
              <Badge color={T.blue}>{participantCount} connected</Badge>
            </div>
            <div style={{ position: "relative", height: 80, background: T.surface, borderRadius: 10, overflow: "hidden", border: `1px solid ${T.border}` }}>
              {/* Simplified continent shapes as dot clusters */}
              {[
                { x: 25, y: 30, label: "Americas", count: 156 },
                { x: 48, y: 25, label: "Europe", count: 134 },
                { x: 55, y: 40, label: "Africa", count: 42 },
                { x: 65, y: 30, label: "Asia", count: 189 },
                { x: 82, y: 55, label: "Oceania", count: 67 },
              ].map((r, i) => (
                <div key={i} style={{ position: "absolute", left: `${r.x}%`, top: `${r.y}%`, transform: "translate(-50%,-50%)" }}>
                  <div style={{ width: Math.max(12, r.count / 8), height: Math.max(12, r.count / 8), borderRadius: "50%", background: T.purple + "30", border: `1.5px solid ${T.purple}50`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <span style={{ fontSize: 7, fontWeight: 800, color: T.purple }}>{r.count}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Live comments + hearts */}
          <Card style={{ marginBottom: 12, padding: "12px 14px", position: "relative", overflow: "hidden", minHeight: 120 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
              <span style={{ fontSize: 12, fontWeight: 600 }}>Live from participants</span>
              <button onClick={() => setShowComments(!showComments)} style={{ background: "none", border: "none", color: T.accent, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>{showComments ? "Hide" : "Show"}</button>
            </div>

            {/* Floating hearts */}
            {hearts.map(h => (
              <div key={h.id} style={{ position: "absolute", left: `${h.x}%`, bottom: 10, fontSize: 16, animation: "floatUp 3s ease-out forwards", opacity: 0, pointerEvents: "none" }}>💜</div>
            ))}

            {showComments && comments.map((c, i) => (
              <div key={c.id} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "6px 0", borderBottom: i < comments.length - 1 ? `1px solid ${T.border}` : "none", animation: "slideUp .3s ease" }}>
                <div style={{ width: 24, height: 24, borderRadius: "50%", background: T.accentDim, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, flexShrink: 0 }}>🌍</div>
                <div>
                  <span style={{ fontSize: 10, color: T.textDim }}>{c.name}</span>
                  <p style={{ fontSize: 12, color: T.text, marginTop: 1 }}>"{c.text}"</p>
                </div>
              </div>
            ))}
          </Card>
          ...
          {/* Condition breakdown */}
          <Card style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Improvement by condition</div>
            {[
              { label: "Chronic pain", pct: Math.round(improvementPct * 1.1), icon: "🔧" },
              { label: "Migraine", pct: Math.round(improvementPct * 1.15), icon: "🧠" },
              { label: "Stress & anxiety", pct: Math.round(improvementPct * 1.2), icon: "🌀" },
              { label: "Back & joint", pct: Math.round(improvementPct * 0.95), icon: "💪" },
            ].map((c, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 0", borderBottom: i < 3 ? `1px solid ${T.border}` : "none" }}>
                <span style={{ fontSize: 11 }}>{c.icon}</span>
                <span style={{ fontSize: 11, fontWeight: 600, flex: 1 }}>{c.label}</span>
                <div style={{ width: 50, height: 5, borderRadius: 3, background: T.border, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${c.pct}%`, background: T.green, borderRadius: 3, transition: "width .5s" }} />
                </div>
                <span style={{ fontSize: 11, fontWeight: 700, color: T.green, minWidth: 28, textAlign: "right" }}>{c.pct}%</span>
              </div>
            ))}
          </Card>
        </div>
...
const HealerGroupPostSession = ({ onHome }) => {
  const participantCount = Math.floor(Math.random() * 100) + 480;
  const improvedPct = Math.floor(Math.random() * 8) + 58;
  const avgDrop = (Math.random() * 1.2 + 3.4).toFixed(1);
  const totalHealed = Math.round(participantCount * improvedPct / 100);
      ...
      <ScreenWrap>
        <div style={{ animation: "slideUp .5s ease" }}>
          ...
          <div style={{ textAlign: "center", padding: "16px 0 20px" }}>
            <div style={{ fontSize: 48, marginBottom: 8 }}>🌍</div>
            <h2 style={{ fontSize: 22, fontWeight: 800, marginBottom: 4 }}>Group session complete</h2>
            <p style={{ fontSize: 13, color: T.textMuted }}>Thank you for your contribution. Here's what happened.</p>
          </div>

          {/* Big stats */}
          <div style={{ display: "flex", gap: 0, marginBottom: 16, borderRadius: 16, overflow: "hidden", border: `1px solid ${T.purple}20` }}>
            {[
              { value: participantCount, label: "participants", color: T.purple },
              { value: `${improvedPct}%`, label: "improved", color: T.green },
              { value: avgDrop, label: "avg drop", color: T.accent },
            ].map((s, i) => (
              <div key={i} style={{ flex: 1, textAlign: "center", padding: "14px 6px", background: s.color + "08", borderRight: i < 2 ? `1px solid ${T.border}` : "none" }}>
          ...
          {/* Impact statement */}
          <Card style={{ marginBottom: 14, background: T.greenDim, border: `1px solid ${T.green}25`, textAlign: "center", padding: "16px" }}>
            <div style={{ fontSize: 28, marginBottom: 6 }}>💚</div>
            <p style={{ fontSize: 14, fontWeight: 600, color: T.green, marginBottom: 2 }}>{totalHealed} people felt better because of this session</p>
            <p style={{ fontSize: 12, color: T.textMuted }}>That's real pain, reduced in real time, measured and verified.</p>
          </Card>

          {/* Improvement by condition */}
          <Card style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 10 }}>Results by condition</div>
            {[
              { label: "Chronic pain", pct: improvedPct + 5, participants: Math.round(participantCount * 0.3), avgDrop: "4.1", icon: "🔧" },
              { label: "Migraine", pct: improvedPct + 8, participants: Math.round(participantCount * 0.22), avgDrop: "4.6", icon: "🧠" },
              { label: "Stress & anxiety", pct: improvedPct + 10, participants: Math.round(participantCount * 0.2), avgDrop: "4.9", icon: "🌀" },
              { label: "Back & joint", pct: improvedPct + 2, participants: Math.round(participantCount * 0.18), avgDrop: "3.8", icon: "💪" },
              { label: "Other", pct: improvedPct - 5, participants: Math.round(participantCount * 0.1), avgDrop: "2.9", icon: "✦" },
            ].map((c, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0", borderBottom: i < 4 ? `1px solid ${T.border}` : "none" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 2 }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, color: T.green }}>{c.pct}%</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <div style={{ flex: 1, height: 4, borderRadius: 2, background: T.border, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${c.pct}%`, background: T.green, borderRadius: 2 }} />
                    </div>
                    <span style={{ fontSize: 9, color: T.textMuted }}>{c.participants} · −{c.avgDrop}pts</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <Card style={{ marginBottom: 14, padding: "14px 16px" }}>
            <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Group improvement over 30 minutes</div>
            <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: 50 }}>
              {[8, 14, 22, 31, 38, 44, 49, 54, 58, improvedPct].map((v, i) => (
                <div key={i} style={{ flex: 1, height: `${(v / 70) * 100}%`, background: `linear-gradient(180deg, ${T.green}90, ${T.green}30)`, borderRadius: 3, minHeight: 3 }} />
              ))}
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
              <span style={{ fontSize: 9, color: T.textDim }}>Start</span>
              <span style={{ fontSize: 9, color: T.textDim }}>15 min</span>
              <span style={{ fontSize: 9, color: T.textDim }}>End</span>
            </div>
          </Card>

          {/* Participant highlights */}
          <Card style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>What participants said</div>
            {[
              { name: "Sarah, UK", text: "My migraine went from 7 to 2. I'm crying.", severity: "7 → 2" },
              { name: "James, US", text: "The back pain just... faded. Unbelievable.", severity: "8 → 3" },
              { name: "Ana, Brazil", text: "First time trying this. I'm speechless.", severity: "6 → 1" },
            ].map((q, i) => (
              <div key={i} style={{ padding: "8px 0", borderBottom: i < 2 ? `1px solid ${T.border}` : "none" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 2 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: T.text }}>{q.name}</span>
                  <Badge color={T.green}>{q.severity}</Badge>
                </div>
                <p style={{ fontSize: 12, color: T.textMuted, fontStyle: "italic" }}>"{q.text}"</p>
              </div>
            ))}
          </Card>
          ...
          <Card style={{ marginBottom: 14, background: `linear-gradient(135deg, ${T.greenDim}, ${T.accentDim})`, border: `1px solid ${T.green}25` }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: T.green }}>Session payment</div>
                <div style={{ fontSize: 11, color: T.textMuted, marginTop: 2 }}>Paid within 48 hours</div>
              </div>
              <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 28, fontWeight: 800, color: T.green }}>$100</div>
            </div>
          </Card>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px 0 16px" }}>
            <span style={{ fontSize: 14 }}>🌍</span>
            <span style={{ fontSize: 11, color: T.textMuted }}>Participants from 42 countries · {Math.round(participantCount * 0.62)} reported 70%+ reduction</span>
          </div>

          <Btn onClick={onHome} full>Back to dashboard</Btn>
        </div>
      </ScreenWrap>
    </>
  );
...
const TierScreen = ({ onSelect, onGroup, onBack }) => (
  <>
    <Header left={<BackBtn onClick={onBack} />} center={<span style={{ fontSize: 13, color: T.textMuted, fontWeight: 500 }}>Paid Sessions</span>} />
    <ScreenWrap>
      <div style={{ animation: "slideUp .4s ease" }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 4 }}>Choose your session tier</h2>
        <p style={{ fontSize: 13, color: T.textMuted, marginBottom: 16 }}>All tiers are Paid Sessions with a verified healer.</p>
        ...
        <div style={{ animation: "slideUp .4s ease" }}>
          ...
          <Card style={{ marginBottom: 16, border: `1px solid ${tier?.color || T.accent}30` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <span style={{ fontSize: 15, fontWeight: 600 }}>{tier?.name || "Paid Session"}</span>
...
const sessionHistory = [
  { date: "Mar 15, 2026", issue: "Chronic Pain", before: 7, after: 3, dur: "14:32", alias: "Ember", type: "free", resets: 2, curve: [7, 7, 6, 5, 4, 3, 3], moments: [{ time: "2:10", text: "I think something is shifting" }, { time: "5:45", text: "The sharpness is definitely less" }, { time: "11:20", text: "It's down to maybe a 3" }], followUp: "Held at 3/10 after 24hrs" },
  { date: "Mar 10, 2026", issue: "Tension", before: 5, after: 2, dur: "11:05", alias: "Solace", type: "paid", resets: 1, curve: [5, 5, 4, 3, 2, 2], moments: [{ time: "3:30", text: "Feeling warmth" }, { time: "7:15", text: "Much better" }], followUp: "Improved to 1/10 after 24hrs" },
  { date: "Feb 28, 2026", issue: "Emotional", before: 6, after: 4, dur: "5:00", alias: "Lumen", type: "free", resets: 0, curve: [6, 6, 5, 5, 4], moments: [{ time: "3:00", text: "Not sure I feel anything" }], followUp: "Unchanged at 4/10" },
];
...
const HistoryScreen = () => {
  ...
  if (detail !== null) {
    ...
    const drop = s.before - s.after;
    const pctDrop = Math.round((drop / Math.max(s.before, 1)) * 100);
    const sevColor = (v) => v <= 2 ? T.green : v <= 4 ? T.accent : v <= 6 ? T.warm : T.danger;
    return (
      <ScreenWrap>
        <div style={{ animation: "slideUp .3s ease" }}>
          ...
          {/* Hero */}
          <div style={{ textAlign: "center", marginBottom: 16 }}>
            <div style={{ fontSize: 36, marginBottom: 6 }}>{drop >= 3 ? "✨" : drop > 0 ? "💫" : "🙏"}</div>
            <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 2 }}>{s.issue}</h2>
            <p style={{ fontSize: 13, color: T.textMuted }}>{s.date} · {s.dur} · Healer: {s.alias}</p>
            <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 8 }}>
              <Badge color={s.type === "paid" ? T.blue : T.accent}>{s.type === "paid" ? "Paid session" : "Free session"}</Badge>
              {s.resets > 0 && <Badge color={T.purple}>{s.resets} clock reset{s.resets > 1 ? "s" : ""}</Badge>}
            </div>
          </div>

          {/* Big result */}
          <Card style={{ marginBottom: 14, background: `linear-gradient(135deg, ${drop >= 3 ? T.greenDim : T.accentDim}, ${T.purpleDim})`, border: `1px solid ${drop >= 3 ? T.green : T.accent}20`, textAlign: "center", padding: "16px" }}>
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 36, fontWeight: 800, color: drop >= 3 ? T.green : T.accent }}>{pctDrop}%</div>
            <p style={{ fontSize: 12, color: T.textMuted, marginTop: 2 }}>reduction · {s.before}/10 → {s.after}/10</p>
            {drop > 0 && (() => {
              const avgDrop = 3.2;
              const isBest = drop === Math.max(...sessionHistory.map(sh => sh.before - sh.after));
              const aboveAvg = drop > avgDrop ? Math.round(((drop - avgDrop) / avgDrop) * 100) : 0;
              return (
                <div style={{ marginTop: 10, padding: "6px 14px", borderRadius: 20, display: "inline-flex", alignItems: "center", gap: 4, background: isBest ? T.greenDim : T.accentDim, border: `1px solid ${isBest ? T.green : T.accent}25` }}>
                  <span style={{ fontSize: 12 }}>{isBest ? "🏆" : "📈"}</span>
                  <span style={{ fontSize: 11, fontWeight: 600, color: isBest ? T.green : T.accent }}>{isBest ? "Your best session" : aboveAvg > 0 ? `${aboveAvg}% above average` : "Matching average"}</span>
                </div>
              );
            })()}
          </Card>
          ...
          {/* Before/After body maps */}
          <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
            <div style={{ flex: 1, background: T.card, borderRadius: 14, padding: 10, border: `1px solid ${T.border}` }}><div style={{ textAlign: "center", marginBottom: 6, fontSize: 10, fontWeight: 600, color: T.textMuted, letterSpacing: 1 }}>BEFORE</div><BodyMap side="front" pins={[{ id: 1, x: 52, y: 55, side: "front", severity: s.before }]} small /></div>
            <div style={{ flex: 1, background: T.card, borderRadius: 14, padding: 10, border: `1px solid ${drop >= 3 ? T.green : T.accent}25` }}><div style={{ textAlign: "center", marginBottom: 6, fontSize: 10, fontWeight: 600, color: drop >= 3 ? T.green : T.accent, letterSpacing: 1 }}>AFTER</div><BodyMap side="front" pins={[{ id: 1, x: 52, y: 55, side: "front", severity: s.after }]} small /></div>
          </div>

          {/* Severity curve */}
          <Card style={{ marginBottom: 14, padding: "14px 16px" }}>
            <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Severity over time</div>
            <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: 50 }}>
              {s.curve.map((v, i) => (
                <div key={i} style={{ flex: 1, height: `${(v / 10) * 100}%`, background: `linear-gradient(180deg, ${sevColor(v)}90, ${sevColor(v)}30)`, borderRadius: 3, minHeight: 3, transition: "height .3s" }} />
              ))}
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
              <span style={{ fontSize: 9, color: T.textDim }}>Start</span>
          ...
          {/* Key moments */}
          <Card style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Key moments</div>
            <div style={{ position: "relative", paddingLeft: 20 }}>
              <div style={{ position: "absolute", left: 6, top: 6, bottom: 6, width: 2, background: `linear-gradient(180deg, ${T.warm}, ${T.green})`, borderRadius: 1 }} />
              {s.moments.map((m, i) => (
                <div key={i} style={{ position: "relative", paddingBottom: i < s.moments.length - 1 ? 12 : 0 }}>
                  <div style={{ position: "absolute", left: -17, top: 3, width: 14, height: 14, borderRadius: "50%", background: T.card, border: `2px solid ${T.accent}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 7 }}>💬</div>
                  <div style={{ paddingLeft: 8 }}>
                    <span style={{ fontSize: 10, color: T.textDim }}>{m.time}</span>
                    <p style={{ fontSize: 12, color: T.text, fontStyle: "italic", marginTop: 1 }}>"{m.text}"</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Follow-up */}
          <Card style={{ marginBottom: 14, background: s.followUp.includes("Improved") || s.followUp.includes("Held") ? T.greenDim : T.warmDim, border: `1px solid ${s.followUp.includes("Improved") || s.followUp.includes("Held") ? T.green : T.warm}20` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 14 }}>{s.followUp.includes("Improved") || s.followUp.includes("Held") ? "✅" : "⏳"}</span>
              <div>
                <div style={{ fontSize: 12, fontWeight: 600 }}>24-hour follow-up</div>
                <div style={{ fontSize: 11, color: T.textMuted }}>{s.followUp}</div>
              </div>
            </div>
          </Card>
          ...
          {/* Session details */}
          <Card style={{ marginBottom: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ fontSize: 12, color: T.textMuted }}>Duration</span><span style={{ fontSize: 12, fontWeight: 600 }}>{s.dur}</span></div>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ fontSize: 12, color: T.textMuted }}>Clock resets</span><span style={{ fontSize: 12, fontWeight: 600 }}>{s.resets}</span></div>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ fontSize: 12, color: T.textMuted }}>Session type</span><span style={{ fontSize: 12, fontWeight: 600 }}>{s.type === "paid" ? "Paid (verified healer)" : "Free (test healer)"}</span></div>
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontSize: 12, color: T.textMuted }}>Healer</span><span style={{ fontSize: 12, fontWeight: 600 }}>{s.alias}</span></div>
          </Card>
        </div>
      </ScreenWrap>
    );
...
const ReferralScreen = ({ onBack }) => {
  ...
  const referralLink = "ennie.app/ref/charlie42";
  const referrals = [
    { name: "Sarah M.", type: "patient", status: "active", earned: 25, stage: "3 sessions completed" },
    { name: "Dr. James K.", type: "healer", status: "qualifying", earned: 0, stage: "28/50 effective sessions" },
    { name: "Taylor R.", type: "patient", status: "signed_up", earned: 0, stage: "Signed up · no session yet" },
    { name: "Alex B.", type: "patient", status: "active", earned: 25, stage: "Month 2 of 3 (10% rev share)" },
    { name: "Mia L.", type: "healer", status: "qualifying", earned: 0, stage: "12/50 effective sessions" },
  ];
  const totalCredits = referrals.filter(r => r.type === "patient").reduce((s, r) => s + r.earned, 0);
  const totalCash = referrals.filter(r => r.type === "healer" && r.status === "paid").reduce((s, r) => s + r.earned, 0);
  const pipeline = referrals.filter(r => r.status === "qualifying" || r.status === "signed_up").length;
  const filtered = activeTab === "all" ? referrals : referrals.filter(r => r.type === activeTab);
  return (
    <>
      <Header left={<BackBtn onClick={onBack} />} center={<span style={{ fontSize: 13, color: T.textMuted, fontWeight: 500 }}>Referral Program</span>} />
      <ScreenWrap>
        <div style={{ animation: "slideUp .4s ease" }}>
          {/* Earnings summary */}
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            <Card style={{ flex: 1, textAlign: "center", padding: 12 }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: T.accent }}>${totalCredits}</div>
              <div style={{ fontSize: 10, color: T.textMuted }}>Credits earned</div>
            </Card>
            <Card style={{ flex: 1, textAlign: "center", padding: 12 }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: T.green }}>${totalCash}</div>
              <div style={{ fontSize: 10, color: T.textMuted }}>Cash earned</div>
            </Card>
            <Card style={{ flex: 1, textAlign: "center", padding: 12 }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: T.purple }}>{pipeline}</div>
              <div style={{ fontSize: 10, color: T.textMuted }}>In pipeline</div>
            </Card>
          </div>

          {/* Share link */}
          <Card style={{ marginBottom: 14, border: `1px solid ${T.accent}25` }}>
            <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Your referral link</div>
            <div style={{ display: "flex", gap: 6, alignItems: "center", padding: "8px 12px", borderRadius: 10, background: T.surface, border: `1px solid ${T.border}`, marginBottom: 10 }}>
              <span style={{ flex: 1, fontSize: 12, color: T.accent, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{referralLink}</span>
              <button onClick={() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }} style={{ padding: "4px 10px", borderRadius: 6, border: `1px solid ${T.accent}30`, background: T.accentDim, color: T.accent, fontSize: 10, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>{copied ? "Copied ✓" : "Copy"}</button>
            </div>
            <div style={{ display: "flex", gap: 6 }}>
              {[{ label: "Message", icon: "💬" }, { label: "Social", icon: "📱" }, { label: "Email", icon: "📧" }, { label: "QR", icon: "📷" }].map(b => (
                <Btn key={b.label} variant="secondary" small style={{ flex: 1, flexDirection: "column", gap: 2, padding: "8px 4px" }}>
          ...
          {/* Two tracks */}
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            <Card style={{ flex: 1, padding: 12, border: `1px solid ${T.blue}20` }}>
              <div style={{ fontSize: 14, marginBottom: 4 }}>👤</div>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 2 }}>Refer a patient</div>
              <div style={{ fontSize: 11, color: T.blue, fontWeight: 600, marginBottom: 4 }}>$25 credit</div>
              <div style={{ fontSize: 10, color: T.textMuted, lineHeight: 1.4 }}>+ 10% of their first 3 months of paid sessions</div>
            </Card>
            <Card style={{ flex: 1, padding: 12, border: `1px solid ${T.green}20` }}>
              <div style={{ fontSize: 14, marginBottom: 4 }}>🌿</div>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 2 }}>Refer a healer</div>
              <div style={{ fontSize: 11, color: T.green, fontWeight: 600, marginBottom: 4 }}>$500 cash</div>
              <div style={{ fontSize: 10, color: T.textMuted, lineHeight: 1.4 }}>Paid after they complete 50 effective sessions</div>
            </Card>
          </div>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
            {[{ id: "all", label: "All" }, { id: "patient", label: "Patients" }, { id: "healer", label: "Healers" }].map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ padding: "4px 12px", borderRadius: 16, fontSize: 11, fontWeight: 600, border: `1px solid ${activeTab === t.id ? T.accent + "50" : T.border}`, background: activeTab === t.id ? T.accentDim : "transparent", color: activeTab === t.id ? T.accent : T.textMuted, cursor: "pointer" }}>{t.label}</button>
            ))}
          </div>
          {filtered.map((r, i) => {
            const progress = r.type === "healer" ? parseInt(r.stage) || 0 : r.status === "active" ? 100 : r.status === "signed_up" ? 30 : 60;
            const statusColor = r.status === "active" ? T.green : r.status === "qualifying" ? T.purple : T.warm;
            return (
              <Card key={i} style={{ marginBottom: 8, padding: "12px 14px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600 }}>{r.name}</div>
                    <div style={{ fontSize: 10, color: T.textMuted }}>{r.stage}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <Badge color={statusColor}>{r.status === "active" ? "Active" : r.status === "qualifying" ? "Qualifying" : "Signed up"}</Badge>
                    {r.earned > 0 && <div style={{ fontSize: 11, fontWeight: 700, color: T.green, marginTop: 2 }}>+${r.earned}</div>}
                  </div>
                </div>
          ...
          <Label>HOW IT WORKS</Label>
          {[
            { num: "1", text: "Share your unique link with friends or healers" },
            { num: "2", text: "They sign up and start using Ennie" },
            { num: "3", text: "You earn credits (patients) or cash (healers) as they hit milestones" },
          ].map((s, i) => (
            <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 10 }}>
              <div style={{ width: 24, height: 24, borderRadius: "50%", background: T.accentDim, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: T.accent, flexShrink: 0 }}>{s.num}</div>
              <p style={{ fontSize: 12, color: T.text, lineHeight: 1.5 }}>{s.text}</p>
            </div>
          ))}
        </div>
      </ScreenWrap>
...
const ProfileScreen = ({ role, setRole, darkMode, setDarkMode }) => (
  <ScreenWrap>
    <div style={{ animation: "slideUp .4s ease" }}>
      ...
      {role === "case" && (
        <>
          <Divider />
          <Label>YOUR HEALTH PROFILE</Label>
          <Card style={{ marginBottom: 10 }}>
            <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Primary conditions</div>
            {[
              { label: "Chronic pain", icon: "🔧", sessions: 3 },
              { label: "Migraine", icon: "🧠", sessions: 1 },
            ].map((c, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0", borderBottom: i < 1 ? `1px solid ${T.border}` : "none" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 12 }}>{c.icon}</span>
                  <span style={{ fontSize: 12, fontWeight: 500 }}>{c.label}</span>
                </div>
                <span style={{ fontSize: 10, color: T.textMuted }}>{c.sessions} session{c.sessions > 1 ? "s" : ""}</span>
              </div>
            ))}
          </Card>
          <Label>SESSION PREFERENCES</Label>
          <Card style={{ marginBottom: 10 }}>
            <Toggle on={true} onToggle={() => {}} label="Default to voice mode in sessions" />
          </Card>
          <Card style={{ marginBottom: 10 }}>
            <Toggle on={false} onToggle={() => {}} label="Auto-join group sessions I've booked" />
          </Card>
        </>
      )}
      ...
      {role === "healer" && (
        <>
          <Divider />
          <Label>HEALING PROFILE</Label>
          <Card style={{ marginBottom: 10 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div><Label>MODALITY</Label><Input placeholder="Energy healing" style={{ marginBottom: 0 }} /></div>
              <div><Label>TIMEZONE</Label><Input placeholder="Australia/Melbourne" style={{ marginBottom: 0 }} /></div>
              <div><Label>BIO (visible to the platform)</Label><Input placeholder="Brief description of your healing approach..." style={{ marginBottom: 0 }} /></div>
            </div>
          </Card>
          <Label>AVAILABILITY</Label>
          <Card style={{ marginBottom: 10 }}>
            <Toggle on={true} onToggle={() => {}} label="Available for 1:1 sessions" />
            <div style={{ height: 8 }} />
            <Toggle on={true} onToggle={() => {}} label="Available for group healing sessions" />
            <div style={{ height: 8 }} />
            <Toggle on={false} onToggle={() => {}} label="Skill-build mode (focus on one condition)" />
          </Card>
        </>
      )}
      ...
      {role === "case" && <Card style={{ marginBottom: 10 }}><Toggle on={true} onToggle={() => {}} label="Group session starting soon" /></Card>}
      {role === "case" && <Card style={{ marginBottom: 10 }}><Toggle on={false} onToggle={() => {}} label="Queue position updates" /></Card>}
      {role === "healer" && <Card style={{ marginBottom: 10 }}><Toggle on={true} onToggle={() => {}} label="New case matches" /></Card>}
      {role === "healer" && <Card style={{ marginBottom: 10 }}><Toggle on={true} onToggle={() => {}} label="Group session reminders" /></Card>}
      <Card style={{ marginBottom: 10 }}><Toggle on={false} onToggle={() => {}} label="Marketing updates" /></Card>
      <Divider />
      <Label>APPEARANCE & ACCESSIBILITY</Label>
      <Card style={{ marginBottom: 10 }}><Toggle on={darkMode} onToggle={() => setDarkMode?.(!darkMode)} label="Dark mode" /></Card>
      <Divider />
      ...
      {role === "case" && (
        <>
          <Card style={{ marginBottom: 10 }}>
            ...
            {[
              { date: "Mar 28, 2026", type: "Paid — Today", amount: "$350", ref: "EN-2026-48291", status: "completed" },
              { date: "Mar 20, 2026", type: "Paid — Line", amount: "$50", ref: "EN-2026-37124", status: "completed" },
      ...
      <ScreenWrap>
        <div style={{ animation: "slideUp .3s ease" }}>
          ...
          <Card style={{ background: "#FAFAFA", marginBottom: 14 }}>
            ...
            {[{ label: "Free tier", w: freeW }, { label: "Paid — Today", w: todayW }, { label: "Paid — Week", w: weekW }].map(r => (
...
const QueueStatusCard = ({ tier = "free", userCondition = "migraine", atFront, onExpand, onReady, onUpgrade, onLeave }) => {
  ...
  const tiers = [
    ...
    { id: "line", name: "Paid — Line", price: "$50", wait: "3–4 weeks", color: T.accent },
    { id: "week", name: "Paid — This Week", price: "$150", wait: "1–7 days", color: T.blue },
    { id: "today", name: "Paid — Today", price: "$350", wait: "< 2 hrs", color: T.warm },
  ];
...
export default function EnnieApp() {
  ...
  const [bookedGroupSession, setBookedGroupSession] = useState(null);
  const [selectedGroupSessions, setSelectedGroupSessions] = useState([]);
  const [paymentFrom, setPaymentFrom] = useState("tiers");
  const [wasGroupSession, setWasGroupSession] = useState(false);
  const [darkMode, setDarkMode] = useState(false);

  const tabbedScreens = ["caseHome", "healerHome", "adminHome"];
  const go = (s) => {
    if (tabbedScreens.includes(s) && !tabbedScreens.includes(screen)) setTab("home");
    setScreen(s);
  };
  ...
  const startPaidSession = (tier) => { setSelectedTier(tier); setPaymentFrom("routing"); go("payment"); };
  const startGroupSession = () => { setBaselinePins(pins.map(p => ({ ...p }))); go("groupWaiting"); };
  const endSession = () => { setWasGroupSession(false); setFinalPins(pins.map(p => ({ ...p, severity: Math.max(0, p.severity - Math.floor(Math.random() * 4 + 2)) }))); go("sessionEnd"); };
  ...
  const screenMap = [
    ...
    { group: "Healer Journey", color: T.warm, items: [
      ...
      { id: "healerGroupInvite", num: "G-H1", label: "Group Healing Invite", star: true },
      { id: "healerGroupSession", num: "G-H2", label: "Healer Group Session", star: true },
      { id: "healerGroupPost", num: "G-H3", label: "Healer Group Post-Session", star: true },
    ]},
    ...
    { group: "Group Healing", color: T.purple, items: [
      ...
      { id: "groupWaiting", num: "G3b", label: "Group Waiting Room", star: true },
    ...
    { group: "Shared / Account", color: T.textMuted, items: [
      ...
      { id: "referrals", num: 24, label: "Referral Program" },
    ]},
  ...
  const renderScreen = () => {
    switch (screen) {
      ...
      case "intake": return <IntakeScreen pins={pins} setPins={setPins} onJoinQueue={() => { if (pins.length > 0) { const topPin = [...pins].sort((a,b) => a.y - b.y)[0]; setUserCondition(topPin.y < 25 ? "migraine" : topPin.y < 45 ? "chronic_back" : topPin.y < 60 ? "arthritis" : "anxiety"); } go("routing"); }} onPaidSession={() => go("routing")} onIneligible={() => go("ineligible")} onBack={() => go("consent")} />;
      case "ineligible": return <IneligibleScreen onPaid={(t) => { setBaselinePins(pins.map(p => ({ ...p }))); setSelectedTier(t); setPaymentFrom("ineligible"); go("payment"); }} onGroup={() => go("groupSchedule")} onClose={() => go("caseHome")} />;
      case "routing": return <RoutingScreen eligible={true} onFree={startFreeSession} onPaid={(t) => startPaidSession(t)} onGroup={() => go("groupSchedule")} onBack={() => go("intake")} />;
      case "queue": return <QueueScreen selectedTier={selectedTier?.name === "Today" ? "today" : selectedTier?.name === "Week" ? "week" : selectedTier?.name === "Line" ? "line" : "free"} userCondition={userCondition} atFront={atQueueFront} onReady={() => go("symptomConfirm")} onGoHome={() => { setInQueue(true); setQueueTier(selectedTier?.name === "Today" ? "today" : selectedTier?.name === "Week" ? "week" : selectedTier?.name === "Line" ? "line" : "free"); go("caseHome"); }} onLeave={() => { setInQueue(false); setAtQueueFront(false); go("caseHome"); }} onUpgrade={(t) => { setSelectedTier(t); setPaymentFrom("queue"); go("payment"); }} />;
      ...
      case "sessionEnd": return <SessionEndScreen baselinePins={baselinePins} finalPins={finalPins} isGroup={wasGroupSession} onHome={() => go("caseHome")} onThankYou={() => go("share")} onNoResult={() => go("noResult")} />;
      ...
      case "followUp": return <FollowUpScreen onPositive={() => go("caseHome")} onNeutral={() => go("noResult")} pins={finalPins.length > 0 ? finalPins : [{ id: 1, x: 52, y: 55, side: "front", severity: 3 }]} setPins={setFinalPins} />;
      ...
      case "healerGroupInvite": return <HealerGroupInviteScreen onCommit={() => go("healerHome")} onBack={() => go("healerHome")} />;
      case "healerGroupSession": return <HealerGroupSessionScreen onEnd={() => go("healerGroupPost")} />;
      case "healerGroupPost": return <HealerGroupPostSession onHome={() => go("healerHome")} />;
      case "adminHome": return <AdminScreen onBack={() => go("landing")} />;
      case "tiers": return <TierScreen onSelect={(t) => { setSelectedTier(t); setPaymentFrom("tiers"); go("payment"); }} onGroup={() => go("groupSchedule")} onBack={() => go("caseHome")} />;
      case "groupSchedule": return <GroupScheduleScreen onSingle={(s) => { setSelectedGroupSessions(s ? [s] : []); go("groupConfirm"); }} onSubscribe={(s) => { setSelectedGroupSessions(s || []); go("groupConfirm"); }} onBack={() => go("caseHome")} />;
      case "groupConfirm": return <GroupConfirmScreen sessions={selectedGroupSessions.length > 0 ? selectedGroupSessions : [groupSessions[0]]} isSub={selectedGroupSessions.length > 1} onIntake={() => { setPins([]); setBookedGroupSession(true); go("groupIntake"); }} onBack={() => go("groupSchedule")} />;
      case "groupIntake": return <GroupIntakeScreen pins={pins} setPins={setPins} onDone={() => { setBaselinePins(pins.map(p => ({ ...p }))); go("groupWaiting"); }} onBack={() => go("groupConfirm")} />;
      case "groupWaiting": return <GroupWaitingRoom onStart={() => go("groupSession")} onBack={() => go("groupIntake")} />;
      case "groupSession": return <GroupSessionScreen pins={pins.length > 0 ? pins : [{ id: 1, x: 52, y: 55, side: "front", severity: 6 }]} setPins={setPins} onEnd={() => { setWasGroupSession(true); setFinalPins(pins.map(p => ({ ...p, severity: Math.max(0, p.severity - Math.floor(Math.random() * 3 + 1)) }))); go("sessionEnd"); }} />;
      case "payment": return <PaymentScreen tier={selectedTier} onPay={() => go("payConfirm")} onBack={() => go(paymentFrom)} />;
      case "payConfirm": return <PaymentConfirmScreen tier={selectedTier} onContinue={() => { setBaselinePins(pins.map(p => ({ ...p }))); setInQueue(true); setQueueTier(selectedTier?.name === "Today" ? "today" : selectedTier?.name === "Week" ? "week" : "line"); go("queue"); }} />;
      case "deleteAccount": return <DeleteScreen onBack={() => go(role === "healer" ? "healerHome" : "caseHome")} />;
      case "referrals": return <ReferralScreen onBack={() => go(role === "healer" ? "healerHome" : "caseHome")} />;
      ...
      case "healerHome":
        return (
          <>
            ...
            />
            {tab === "home" && <HealerHomeScreen onGoOnline={() => go("healerPing")} onSpecializations={() => go("specializations")} onGroupInvite={(action) => action === "joinSession" ? go("healerGroupSession") : go("healerGroupInvite")} onReferrals={() => go("referrals")} />}
            {tab === "earnings" && <HealerEarningsScreen />}
            {tab === "history" && <HistoryScreen />}
            {tab === "support" && <SupportScreen />}
            {tab === "profile" && <><ProfileScreen role={role} darkMode={darkMode} setDarkMode={setDarkMode} setRole={(r) => { setRole(r); if (r === "case") go("caseHome"); else if (r === "admin") go("adminHome"); }} /><div style={{ padding: "0 16px 8px" }}><Btn variant="danger" small full onClick={() => go("deleteAccount")}>Delete account</Btn></div></>}
      ...
      case "adminHome":
        return (
          <>
            ...
            {tab === "profile" && <ProfileScreen role={role} darkMode={darkMode} setDarkMode={setDarkMode} setRole={(r) => { setRole(r); if (r === "case") go("caseHome"); else if (r === "healer") go("healerHome"); }} />}
            ...
            {tab === "home" && (
              <ScreenWrap>
                <div style={{ animation: "slideUp .4s ease" }}>
                  ...
                  {/* Priority 0: Upcoming group sessions */}
                  {bookedGroupSession && (
                    <div style={{ marginBottom: 14 }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <span style={{ fontSize: 14 }}>👥</span>
                          <span style={{ fontSize: 12, fontWeight: 700 }}>Your group sessions</span>
                        </div>
                        <button onClick={() => go("groupSchedule")} style={{ background: "none", border: "none", color: T.purple, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Manage →</button>
                      </div>
                      {[
                        { day: "Today", time: "7:00 PM", focus: "Chronic pain", isToday: true },
                        { day: "Wednesday", time: "12:00 PM", focus: "Stress & anxiety", isToday: false },
                        { day: "Next Monday", time: "7:00 PM", focus: "Chronic pain", isToday: false },
                      ].map((s, i) => (
                        <Card key={i} onClick={s.isToday ? () => { setPins([]); go("groupIntake"); } : undefined} style={{ marginBottom: 6, cursor: s.isToday ? "pointer" : "default", border: `1px solid ${s.isToday ? T.purple + "40" : T.border}`, background: s.isToday ? `${T.purple}06` : T.card, padding: "10px 14px" }}>
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              <span style={{ fontSize: 13, fontWeight: 600, color: s.isToday ? T.purple : T.text }}>{s.day}</span>
                              <span style={{ fontSize: 12, color: T.purple, fontWeight: 500 }}>{s.time}</span>
                              <span style={{ fontSize: 10, color: T.textMuted }}>{s.focus}</span>
                            </div>
                            {s.isToday ? (
                              <button style={{ padding: "3px 10px", borderRadius: 8, border: "none", background: `linear-gradient(135deg, ${T.purple}, ${T.blue})`, color: "#fff", fontSize: 10, fontWeight: 700, cursor: "pointer" }}>Join →</button>
                            ) : (
                              <span style={{ fontSize: 10, color: T.textDim }}>upcoming</span>
                            )}
                          </div>
                        </Card>
                      ))}
                  ...
                  {sessionHistory.length > 0 ? (
                    ...
                    <EmptyState icon="📊" title="No sessions yet" desc="Your progress stats will appear here after your first session." action="Start your first session" onAction={() => { setPins([]); go("intake"); }} />
                  )}
                  ...
                  {/* Record another */}
                  <Card onClick={() => go("share")} style={{ marginBottom: 12, cursor: "pointer", border: `1px solid ${T.purple}25` }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  {/* Referral card */}
                  <Card onClick={() => go("referrals")} style={{ marginBottom: 12, cursor: "pointer", border: `1px solid ${T.green}25`, background: `${T.green}04` }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ width: 36, height: 36, borderRadius: 10, background: T.greenDim, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>🎁</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 12, fontWeight: 600 }}>Refer & earn</div>
            ...
            {tab === "billing" && (
              <ScreenWrap>
                <div style={{ animation: "slideUp .4s ease" }}>
                  ...
                  {[
                    { date: "Mar 28", desc: "Paid — Today", amount: "$350", method: "Visa •••• 4242", ref: "EN-48291", status: "paid" },
                    { date: "Mar 20", desc: "Paid — Line", amount: "$50", method: "Visa •••• 4242", ref: "EN-37124", status: "paid" },
                    { date: "Mar 10", desc: "Group Healing (monthly)", amount: "$19.99", method: "Visa •••• 4242", ref: "EN-28490", status: "paid" },
                    { date: "Feb 28", desc: "Paid — Week", amount: "$150", method: "Apple Pay", ref: "EN-19283", status: "paid" },
                    { date: "Feb 10", desc: "Group Healing (monthly)", amount: "$19.99", method: "Visa •••• 4242", ref: "EN-14720", status: "paid" },
                    { date: "Jan 15", desc: "Paid — Today", amount: "$350", method: "Visa •••• 4242", ref: "EN-09371", status: "refunded" },
                  ].map((p, i) => (
  ...
  return (
    <>
      <a href="#ennie-main" className="skip-to-content">Skip to content</a>
      <div id="ennie-main" role="main" aria-live="polite" className={darkMode ? "ennie-dark" : ""} style={containerStyle}>
      <GlobalCSS />
      {renderScreen()}
